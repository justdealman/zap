...
function slider() {
	var h = $(window).height()*2-$('.core').height()-$('header').height();
	if ( h < 500 ) {
		h = 500;
	}
	if ( h > 1400 ) {
...
function cards() {
	...
	console.log($('.contacts .card.first').position().top);
...
$(document).ready(function() {
	...
	if ( $('.core').length > 0 ) {
		var hhh = Math.ceil($(window).height()/907*999);
		if ( hhh < 1000 ) {
			hhh = 1000;
		}
		if ( hhh > 1500 ) {
